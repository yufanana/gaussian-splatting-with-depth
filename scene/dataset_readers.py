...
def correct_group_delay(wfm, gd, fs):
    ...
    return wfm_correct.reshape(-1)

def generate_points_in_fov(num_pts, center, inner_radius, outer_radius, fov_x, fov_y):
    # Generate random points in spherical coordinates within the specified FoV
    r = np.random.uniform(inner_radius, outer_radius, num_pts)
    theta = np.random.uniform(-fov_x / 2, fov_x / 2, num_pts)  # Horizontal angle
    phi = np.random.uniform(-fov_y, fov_y, num_pts)  # Vertical angle

    # Convert to Cartesian coordinates
    x = center[0] + r * np.cos(phi) * np.sin(theta)
    y = center[1] + r * np.sin(phi)
    z = center[2] + r * np.cos(phi) * np.cos(theta)

    # Keep only points with positive Z values
    positive_z = z > 0
    return np.vstack((x[positive_z], y[positive_z], z[positive_z])).T
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, depth_folder=None):
    ...
    speed_of_sound = 345.11 # m/s
    background_noise = None

    if depth_folder is not None:
        if os.path.exists(os.path.join(depth_folder, "background.csv")):
            background_noise = np.genfromtxt(os.path.join(depth_folder, "background.csv"), delimiter=',').reshape(-1)
            background_noise = correct_group_delay(background_noise, 62.329924, 100000)
            background_noise[:300] = 0

    number_of_cameras = len(cam_extrinsics)
    points_per_camera = round(10000 / number_of_cameras)
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        if depth_folder is not None:
            ...
            # if angle == '0':
            #     angle = '360'
            angle = str(int(angle) + 1)
            sonar_file = f'Flight-{angle.zfill(6)}.csv'
            sonar_wf_file = os.path.join(depth_folder, sonar_file)
            depth = np.genfromtxt(sonar_wf_file, delimiter=',').reshape(-1)
            depth = correct_group_delay(depth, 62.329924, 100000)
            depth[:300] = 0
            if background_noise is not None:
                depth = depth - background_noise
            
            transmit_wf = LFM(f_start=10e3, f_stop=30e3, Fs=100000, T=0.001, V_peak=3, alpha=0.1)

            hilbert_depth = signal.hilbert(depth)
            hilbert_fft = np.fft.fft(hilbert_depth)
            transmit_fft = np.fft.fft(transmit_wf, 1000)
            analytic_data = np.fft.ifft(hilbert_fft * np.conj(transmit_fft))
            analytic_data = np.abs(analytic_data.real + 1j * analytic_data.imag)

            # analytic_data = signal.filtfilt(b, a, analytic_data)
            analytic_data = analytic_data / np.max(analytic_data)
            # analytic_data[:300] = 0

            analytic_mean = np.mean(analytic_data)
            analytic_std = np.std(analytic_data)
            threshold = analytic_mean + 1 * analytic_std

            high_energy_indices = np.where(analytic_data > threshold)[0]
            distance_line = np.linspace(0, (speed_of_sound * 0.01) / 2, len(analytic_data))
            distance_values = distance_line[high_energy_indices]

            generate_points = generate_points_in_fov(points_per_camera, T, distance_values.min(), distance_values.max(), FovX, FovY)

            final_points.append(generate_points)

        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                              image_path=image_path, image_name=image_name, depth_path=sonar_wf_file ,
                              width=width, height=height,
                                depth=analytic_data if depth_folder is not None else None)
        cam_infos.append(cam_info)

    final_points = np.vstack(final_points)

    # # add some more random points to the point cloud
    # num_random_points = 100000
    # random_points = np.random.random((num_random_points, 3)) * 2.0
    # final_points = np.vstack((final_points, random_points))

    sys.stdout.write('\r')
    sys.stdout.write('###### Total number of final cameras: {} ######\n'.format(len(cam_infos)))
    sys.stdout.flush()

    sys.stdout.write('\n')
    if depth_folder is not None:
        return cam_infos, final_points
    else:
        return cam_infos
...
def readColmapSceneInfo(path, images, eval, llffhold=4):
    ...
    if os.path.exists(os.path.join(path, "Sonar_raw")):
        cam_infos_unsorted, final_points = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, 
                                            images_folder=os.path.join(path, reading_dir), depth_folder=os.path.join(path, "Sonar_raw"))
    else:
    ...
    if not os.path.exists(ply_path):
        ...
        shs = np.zeros((final_points.shape[0], 3)) / 255.0
        xyz = np.vstack([xyz, final_points])
        rgb = np.vstack([rgb, SH2RGB(shs) * 255])
...
#     color_folder_path = os.path.join(path, "calibrated_images")
#     # sonar waveform data
...
#         # Create the camera
#         cam_info = CameraInfo(uid=idx, R=extrinsic[:3, :3], T=extrinsic[:3, 3], FovY=fov_y, FovX=fov_x, image=image,
#                               image_path=os.path.join(color_folder_path, file), image_name=str(file), width=width, height=height, wf=wf)
#         cam_list.append(cam_info)
